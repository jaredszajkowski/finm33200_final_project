...
def pull_CRSP_daily_index(
    start_date=START_DATE, end_date=END_DATE, wrds_username=WRDS_USERNAME
):
    """
    Pulls daily CRSP index data from crsp.dsi.

    Returns a DataFrame with columns including vwretd (value-weighted return
    including dividends), which is used to compute monthly market return and
    realized volatility for Bybee et al. (2024) Table I.

    Note: After the CRSP CIZ migration (Jan 2025), the date column in crsp.dsi
    was renamed from 'caldt' to 'date'.
    """
    query = f"""
        SELECT date, vwretd, vwretx, ewretd, ewretx,
               totval, totcnt, usdval, usdcnt, sprtrn, spindx
        FROM crsp.dsi
...
def load_CRSP_daily_index(data_dir=DATA_DIR):
...
def _demo():
    ...
    df_dsi = load_CRSP_daily_index(data_dir=DATA_DIR)
